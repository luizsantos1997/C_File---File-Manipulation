...
int main(void) {
    ...
    FILE *Arquivo; // cria vari√°vel ponteiro para o arquivo
     int i, j;
     printf("Quantas Pessoas vocÍ quer calcular o 'IMC': ");

    scanf("%d", &i);
    
    printf("\n\n");
    struct Cadastrar { // struct para cadastrar as informa√ß√µes necessarias do usuario;
        char nome[100];
        float peso ;
        float altura;
        int idade;
        char sexo;
        float imc; // obtem a massa corporea em uma vari√°vel;
    };
